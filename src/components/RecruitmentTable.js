    ...
    <>
      ...
      <Card className="overflow-x-auto">
        <div className="flex flex-col md:flex-row items-stretch md:items-center mb-4">
          <h1 className="font-bold text-center text-xl text-yellow-600">
            Penerimaan Karyawan Baru
          </h1>
          <div className="ml-auto"></div>
          <Link
            to="/admin/recruitments/create"
            className="inline-block whitespace-no-wrap bg-yellow-400 hover:bg-yellow-600 hover:text-white font-semibold text-sm text-black px-4 py-1 rounded-sm my-2 md:my-0 md:ml-4">
            Tambah Penerimaan
          </Link>
          <a
            target="_blank"
            ...
            <tbody>
              {recruitments.data &&
                recruitments.data.map((recruitment, index) => {
                  ...
                  const overTime =
                    new Date().getTime() - deadlineDate.getTime();
